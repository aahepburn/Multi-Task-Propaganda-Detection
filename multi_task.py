...
def train_mtl_flat(
    ...
    best_f1_scores = {task: 0.0 for task in loaders}
    ...
    for epoch in range(epochs):
        ...
        for task in loaders:
            print(f"\n→ Training task: {task}")
            total_loss = 0.0
            train_loader = loaders[task]

            for batch in tqdm(train_loader, desc=f"[{task.upper()}] Training"):
                input_ids = batch["input_ids"].to(device)
                attention_mask = batch["attention_mask"].to(device)
                labels = batch[f"{task}_labels"].to(device).float()
            ...
            print(f"✓ [{task}] Avg Loss: {avg_loss:.4f}")
        ...
        for task in val_data:
            ...
            with torch.no_grad():
                for batch in tqdm(val_loader, desc=f"[{task.upper()}] Validating"):
                    input_ids = batch["input_ids"].to(device)
                    attention_mask = batch["attention_mask"].to(device)
            ...
            preds_bin = (np.array(all_preds) > 0.35).astype(int)
            macro_f1 = f1_score(y_val, preds_bin, average="macro", zero_division=0)
            print(f"✓ [{task}] Val Macro F1: {macro_f1:.4f}")

            if macro_f1 > best_f1_scores[task]:
                best_f1_scores[task] = macro_f1
                path = f"{task}_MTL_{'-'.join(train_domain)}_to_{'-'.join(test_domain)}.pt"
                torch.save(model.state_dict(), path)
                print(f"★ New best model saved for [{task}] at {path}")
